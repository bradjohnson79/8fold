...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type t JOIN pg_namespace n ON n.oid = t.typnamespace WHERE n.nspname = 'public' AND t.typname = 'EscrowKind') THEN
    CREATE TYPE public."EscrowKind" AS ENUM ('JOB_ESCROW', 'PARTS_MATERIALS');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_type t JOIN pg_namespace n ON n.oid = t.typnamespace WHERE n.nspname = 'public' AND t.typname = 'EscrowStatus') THEN
    CREATE TYPE public."EscrowStatus" AS ENUM ('PENDING', 'FUNDED', 'RELEASED', 'REFUNDED', 'FAILED');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_type t JOIN pg_namespace n ON n.oid = t.typnamespace WHERE n.nspname = 'public' AND t.typname = 'PartsMaterialStatus') THEN
...
DO $$
DECLARE col_id_type text; has_escrow_id boolean; has_currency boolean; has_stripe_ref boolean;
BEGIN
  SELECT data_type INTO col_id_type FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'LedgerEntry' AND column_name = 'id';
  ...
  IF col_id_type = 'text' THEN
    ...
    SELECT "createdAt", "userId", "jobId", "type", "direction", "bucket", "amountCents", 'USD'::public."CurrencyCode", "memo" FROM public."LedgerEntry_old";
  ...
  ELSE
    ...
    IF NOT has_escrow_id THEN ALTER TABLE public."LedgerEntry" ADD COLUMN "escrowId" uuid REFERENCES public."Escrow"("id"); CREATE INDEX IF NOT EXISTS "LedgerEntry_escrowId_idx" ON public."LedgerEntry" ("escrowId"); END IF;
    ...
    IF NOT has_currency THEN ALTER TABLE public."LedgerEntry" ADD COLUMN "currency" public."CurrencyCode" NOT NULL DEFAULT 'USD'::public."CurrencyCode"; END IF;
    ...
    IF NOT has_stripe_ref THEN ALTER TABLE public."LedgerEntry" ADD COLUMN "stripeRef" text; CREATE INDEX IF NOT EXISTS "LedgerEntry_stripeRef_idx" ON public."LedgerEntry" ("stripeRef"); END IF;
  END IF;
END $$;
