...
/**
 * Seed deterministic, NON-mock data to verify the Contractor Dashboard end-to-end.
 *
 * Creates / ensures:
 * - 1 contractor web user (role CONTRACTOR)
 * - 1 approved inventory Contractor record matched by email (Austin, TX; HANDYMAN eligible)
 * - 1 job poster web user (role JOB_POSTER) with phone/email (for contact unlock)
 * - 1 router web user (role ROUTER) (for routed dispatch attribution)
 * - 1 paid job (payment CAPTURED) routed to the contractor via a JobDispatch (PENDING)
 *
 * Usage:
 *   pnpm tsx scripts/seed-contractor-dashboard-e2e.ts
 */
async function main() {
  assertNotProductionSeed("scripts/seed-contractor-dashboard-e2e.ts");
  ...
  const { contractorAccounts } = await import("../apps/api/db/schema/contractorAccount");
  ...
  // Ensure inventory Contractor record exists + is APPROVED and matches user email.
  ...
  // Reset derived artifacts for idempotent runs
  ...
  const dispatchInserted = await db
