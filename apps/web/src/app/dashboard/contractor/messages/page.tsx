...
export default function ContractorMessagesPage() {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold">Messages</h1>
      <p className="mt-1 text-gray-600">Thread list — click a thread to view messages.</p>
        ...
        {threads.length === 0 ? (
          ...
          <ul className="space-y-2">
            {threads.map((t) => (
              <li key={t.id}>
                <Link
                  href={`/dashboard/contractor/messages/${t.id}`}
                  className="block rounded-lg border border-gray-200 p-4 hover:bg-gray-50"
                >
                  <span className="font-medium">{t.title ?? `Job ${t.jobId.slice(0, 8)}`}</span>
