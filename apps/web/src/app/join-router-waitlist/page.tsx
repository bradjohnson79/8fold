...
export default function JoinRouterWaitlistPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const resp = await fetch(`${apiOrigin}/api/public/router-waitlist`, {
        method: "POST",
  ...
  return (
    <div className="min-h-screen bg-8fold-navy">
      <div className="max-w-2xl mx-auto px-4 pt-16 pb-8 text-center">
        <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/20 border border-blue-400/30 text-blue-300 text-xs font-bold tracking-wider uppercase mb-6">
          <span className="w-1.5 h-1.5 rounded-full bg-blue-300 animate-pulse" />
          Phase 2 — Coming Soon
        </span>
        <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
          Join the Router Waitlist
        </h1>
        <p className="mt-4 text-gray-400 text-lg max-w-xl mx-auto">
          Routers coordinate jobs in their communities and earn 8% per completed job.
          Phase 2 router applications open once job posting launches across California.
        </p>
      </div>
      ...
      <div className="max-w-xl mx-auto px-4 pb-24">
        ...
        >
          ...
          >
            {submitting ? "Submitting…" : "Notify Me When Routers Open →"}
          </button>
