...
export const jobs = dbSchema.table(
  "jobs",
  {
    ...
    is_regional: boolean("is_regional").notNull().default(false),
    appraisal_subtotal_cents: integer("appraisal_subtotal_cents").notNull().default(0),
    regional_fee_cents: integer("regional_fee_cents").notNull().default(0),
    tax_rate_bps: integer("tax_rate_bps").notNull().default(0),
    tax_amount_cents: integer("tax_amount_cents").notNull().default(0),
    total_amount_cents: integer("total_amount_cents").notNull().default(0),
    stripe_payment_intent_id: text("stripe_payment_intent_id"),
    stripe_payment_intent_status: text("stripe_payment_intent_status"),
    stripe_charge_id: text("stripe_charge_id"),
    stripe_customer_id: text("stripe_customer_id"),
    stripe_payment_method_id: text("stripe_payment_method_id"),
    stripe_capture_deadline_at: timestamp("stripe_capture_deadline_at", { mode: "date" }),
    stripe_authorized_at: timestamp("stripe_authorized_at", { mode: "date" }),
    stripe_captured_at: timestamp("stripe_captured_at", { mode: "date" }),
    stripe_canceled_at: timestamp("stripe_canceled_at", { mode: "date" }),
    stripe_paid_at: timestamp("stripe_paid_at", { mode: "date" }),
    stripe_refunded_at: timestamp("stripe_refunded_at", { mode: "date" }),
