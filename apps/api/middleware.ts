...
function getAdminOrigin(): string {
  return String(process.env.ADMIN_ORIGIN ?? "").trim().replace(/\/+$/, "");
}

function logMiddlewareFailureOnce(err: unknown): void {
  const KEY = "__8FOLD_API_MIDDLEWARE_ERROR_LOGGED__";
  if ((globalThis as any)[KEY]) return;
  (globalThis as any)[KEY] = true;

  // Production-safe: log only the error message, no env values.
  const msg = err instanceof Error ? err.message : String(err);
  console.error("[api middleware] failure:", msg);
}
...
export function middleware(req: Request) {
  try {
    ...
    // Health endpoints must never be blocked or crash due to config checks.
    // This ensures /api/system/health is usable for production diagnostics.
    if (url.pathname === "/api/system/health" || url.pathname === "/api/health") {
      return NextResponse.next();
    }

    // Fail-fast config checks are useful, but middleware must never take the whole
    // deployment down with an uncaught exception (Edge runtime => MIDDLEWARE_INVOCATION_FAILED).
    try {
      logBootConfigOnce();
    } catch (err) {
      logMiddlewareFailureOnce(err);
      // Continue; handlers can still respond (and will fail-fast where required).
    }
    ...
    function withCors(resp: Response): Response {
      // If ADMIN_ORIGIN is not configured, do not emit an invalid header value.
      if (allowOrigin) {
    ...
    const hasSession = Boolean(
      expectedSecret && providedSecret && providedSecret === expectedSecret && uuidRe.test(adminId),
    );
    ...
    // Absolute last-resort safety: middleware must never crash the edge runtime.
    logMiddlewareFailureOnce(err);
    return NextResponse.next();
  }
}
...
export const config = {
  // This middleware is only for direct admin API calls to apps/api.
  // Keeping the matcher narrow prevents accidental downtime (e.g. health endpoints).
