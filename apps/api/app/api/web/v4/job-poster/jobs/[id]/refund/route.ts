...
export async function POST(
  ...
  try {
    ...
    const result = await refundRoutableUnassignedJobForPoster(id, role.userId);
    return NextResponse.json({ ok: true, ...result });
  } catch (err) {
    const wrapped = err instanceof Error && "status" in err ? (err as V4Error) : internal("V4_REFUND_JOB_FAILED");
