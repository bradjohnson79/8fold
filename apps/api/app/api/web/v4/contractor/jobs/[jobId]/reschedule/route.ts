...
import { rescheduleAppointment } from "@/src/services/v4/contractorJobService";
...
export async function POST(
  ...
  try {
    ...
    const result = await rescheduleAppointment(ctx.internalUser.id, jobId, appointmentAt);
    return NextResponse.json(result);
  } catch (err) {
    const wrapped = err instanceof Error && "status" in err ? (err as V4Error) : internal("V4_RESCHEDULE_APPOINTMENT_FAILED");
