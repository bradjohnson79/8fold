...
export async function GET(req: Request) {
  ...
  const prefs = await getPreferences({ userId: role.userId, role: "ROUTER" });
...
export async function PATCH(req: Request) {
  ...
  const prefs = await updatePreferences({ userId: role.userId, role: "ROUTER", items: body.data.items });
  return NextResponse.json({ ok: true, ...prefs }, { headers: { "cache-control": "no-store" } });
}
