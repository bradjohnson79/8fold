...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await userLifecycleRepo.unsuspendManagedUser({
      userId: id,
    ...
    console.error("[ADMIN_V4_USER_UNSUSPEND_ERROR]", {
      message: error instanceof Error ? error.message : String(error),
    });
    return err(500, "ADMIN_V4_USER_UNSUSPEND_FAILED", "Failed to unsuspend user");
  }
}
