...
export async function POST(req: Request) {
  ...
  try {
    ...
    const result = await db.transaction(async (tx: any) => {
      ...
      assertJobDraftTransition((current as any).status, "REJECTED");
    ...
    return handleApiError(err, "POST /api/admin/job-drafts/:id/reject", { route: "/api/admin/job-drafts/[id]/reject", userId: auth.userId });
  }
}
