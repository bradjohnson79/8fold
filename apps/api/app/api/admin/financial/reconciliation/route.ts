...
export async function GET(req: Request) {
  ...
  try {
    ...
    // Simple variance heuristic:
    // - compare Stripe "available+pending" to internal "escrow fund net - payouts paid".
    // This is not a formal audit, but is sufficient for a warning badge.
    const stripeTotalCents = (() => {
      const b = stripeBalance as any;
      const sum = (arr: any[]) => (Array.isArray(arr) ? arr.reduce((acc, x) => acc + Number(x?.amount ?? 0), 0) : 0);
      const available = sum(b?.available);
      const pending = sum(b?.pending);
      return Number(available + pending);
    })();
    const internalCents =
      Number(ledgerWindow?.[0]?.escrowFundNetCents ?? 0) - Number(ledgerWindow?.[0]?.payoutCreditPaidCents ?? 0);
    const varianceCents = stripeTotalCents - internalCents;
    const warning = Math.abs(varianceCents) > 100;

    return NextResponse.json({
      ok: true,
      data: {
        window: { days, since: since.toISOString() },
        payments: { lifetime: paymentsLifetime?.[0] ?? null, window: paymentsWindow?.[0] ?? null },
        ledger: { lifetime: ledgerLifetime?.[0] ?? null, window: ledgerWindow?.[0] ?? null },
        payoutRequests: { lifetime: payoutReqLifetime?.[0] ?? null, window: payoutReqWindow?.[0] ?? null },
        transfers: { lifetime: transferLifetime?.[0] ?? null, window: transferWindow?.[0] ?? null },
    ...
    return handleApiError(err, "GET /api/admin/financial/reconciliation", { userId: auth.userId });
  }
}
