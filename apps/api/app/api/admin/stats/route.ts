...
export async function GET(req: Request) {
  ...
  try {
    ...
    const [
      ...
      count(db.select({ c: sql<number>`count(*)` }).from(jobs).where(and(eq(jobs.is_mock, false), eq(jobs.status, "CUSTOMER_APPROVED")))),
      count(db.select({ c: sql<number>`count(*)` }).from(jobs).where(and(eq(jobs.is_mock, false), eq(jobs.status, "CUSTOMER_REJECTED")))),
      count(db.select({ c: sql<number>`count(*)` }).from(jobs).where(and(eq(jobs.is_mock, false), eq(jobs.status, "COMPLETION_FLAGGED")))),
      count(db.select({ c: sql<number>`count(*)` }).from(jobs).where(and(eq(jobs.is_mock, false), eq(jobs.status, "COMPLETED_APPROVED")))),
