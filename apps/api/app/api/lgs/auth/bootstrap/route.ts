...
export async function POST(req: Request) {
  ...
  try {
    ...
    const payload = (await req.json().catch(() => null)) as {
      email?: string; password?: string; role?: string; tokenCode?: string;
    } | null;
    ...
    if (!creationTokenValid(tokenCode)) {
      console.warn("[LGS_BOOTSTRAP_DENIED]", { reason: "invalid_token", email, ip });
      return NextResponse.json(
    ...
    return NextResponse.json({ ok: true, data: { created: true, admin: created[0] } }, { status: 201 });
  } catch (error) {
    console.error("[LGS_BOOTSTRAP_ERROR]", { ip, message: error instanceof Error ? error.message : String(error) });
    return NextResponse.json(
