...
import { contractorLaunchList } from "@/db/schema/contractorLaunchList";
...
function buildConfirmationEmail(firstName: string): string {
  return emailWrap(`
    <h1 style="margin:0 0 20px;font-size:22px;font-weight:700;color:#111827;">You're on the 8Fold Contractor Launch List</h1>
...
export async function POST(req: Request) {
  ...
  const existing = await db
    .select({ id: contractorLaunchList.id })
    .from(contractorLaunchList)
    .where(eq(contractorLaunchList.email, normalizedEmail))
    .limit(1);
  ...
  await db.insert(contractorLaunchList).values({
  ...
  void sendTransactionalEmail({
    to: email,
    subject: "You're on the 8Fold Contractor Launch List",
    ...
    console.error("[CONTRACTOR_LAUNCH_LIST_EMAIL_ERROR]", { email, err: String(err) });
  });
