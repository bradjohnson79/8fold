...
import { jobposterWaitlist } from "@/db/schema/jobposterWaitlist";
...
function buildConfirmationEmail(firstName: string): string {
  return emailWrap(`
    ...
    <p style="margin:0 0 14px;font-size:15px;line-height:1.6;color:#374151;">
      Thanks for your interest in posting jobs on 8Fold.
    </p>
    <p style="margin:0 0 14px;font-size:15px;line-height:1.6;color:#374151;">
      We're currently building the California contractor network during Phase 1.
      You'll be notified as soon as job posting opens statewide.
    </p>
...
export async function POST(req: Request) {
  ...
  const existing = await db
    .select({ id: jobposterWaitlist.id })
    .from(jobposterWaitlist)
    .where(eq(jobposterWaitlist.email, email.toLowerCase()))
    .limit(1);
  ...
  await db.insert(jobposterWaitlist).values({
  ...
  void sendTransactionalEmail({
    ...
    text: `Hello ${firstName},\n\nThanks for your interest in posting jobs on 8Fold.\n\nWe're currently building the California contractor network during Phase 1. You'll be notified as soon as job posting opens statewide.\n\n— The 8Fold Team\n\nYou are receiving this because you joined the 8Fold launch network.`,
  }).catch((err) => {
    console.error("[JOBPOSTER_WAITLIST_EMAIL_ERROR]", { email, err: String(err) });
  });
