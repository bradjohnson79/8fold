...
export async function createV4Job(input: z.infer<typeof V4JobCreateBodySchema>) {
  const now = new Date();
  ...
  await db.transaction(async (tx) => {
    ...
    try {
      ...
      if (msg.includes("does not exist") || msg.includes("relation")) {
        await tx.update(jobs).set({ photo_urls: urls, updated_at: now }).where(eq(jobs.id, jobId));
      } else {
        throw photoErr;
      }
    }
