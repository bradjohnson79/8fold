...
export default async function JobPosterDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;

  let data: DetailResp | null = null;
  let err: string | null = null;
  try {
    data = await adminApiFetch<DetailResp>(`/api/admin/v4/job-posters/${encodeURIComponent(id)}`);
  } catch (e) {
    err = e instanceof Error ? e.message : "Failed to load job poster";
  }

  if (!data || err) {
    return (
      <div>
        <h1 style={{ margin: 0, fontSize: 22, fontWeight: 950 }}>Job Poster</h1>
        <p style={{ marginTop: 10, color: "rgba(254,202,202,0.95)" }}>{err ?? "Failed to load"}</p>
        <a href="/job-posters" style={{ color: "rgba(191,219,254,0.95)", textDecoration: "none", fontWeight: 900 }}>
          ← Back to Job Posters
        </a>
      </div>
    );
  ...
  return (
    <div>
      <h1 style={{ margin: 0, fontSize: 22, fontWeight: 950 }}>Job Poster Detail</h1>
