...
export default async function RouterDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    data = await adminApiFetch<DetailResp>(`/api/admin/v4/routers/${encodeURIComponent(id)}`);
  } catch (e) {
    err = e instanceof Error ? e.message : "Failed to load router";
  }

  if (!data || err) {
    return (
      <div>
        <h1 style={{ margin: 0, fontSize: 22, fontWeight: 950 }}>Router</h1>
        <p style={{ marginTop: 10, color: "rgba(254,202,202,0.95)" }}>{err ?? "Failed to load"}</p>
        <a href="/routers" style={{ color: "rgba(191,219,254,0.95)", textDecoration: "none", fontWeight: 900 }}>
          ← Back to Routers
        </a>
      </div>
    );
  ...
  return (
    <div>
      <h1 style={{ margin: 0, fontSize: 22, fontWeight: 950 }}>Router Detail</h1>
