...
export default async function TitleAuditPage({
  searchParams,
}: {
  ...
  return (
    <div>
      <h1 style={{ margin: 0, fontSize: 22, fontWeight: 950, letterSpacing: 0.2 }}>Job Title Quality Audit</h1>
      <p style={{ marginTop: 8, color: "rgba(226,232,240,0.72)", maxWidth: 980 }}>
        Deterministic scoring + rewrite suggestions to remove AI/corporate template phrasing. No LLM randomness.
      </p>
      ...
      <div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "1.1fr 0.9fr", gap: 12, alignItems: "start" }}>
        ...
        <Card title="Batch rewrite" subtitle="Writes are audited as JOB_TITLE_REWRITE_ADMIN.">
          <form action={rewriteSelected} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div style={{ fontSize: 12, color: "rgba(226,232,240,0.62)", lineHeight: 1.45 }}>
              Tip: switch “Only flagged” to “All” to see post-rewrite improvements.
            </div>
          </form>
        </Card>
      </div>
      ...
      <div style={{ marginTop: 12, overflowX: "auto" }}>
        <form action={rewriteSelected}>
          ...
          <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0 }}>
            <thead>
              <tr>
                {["", "Score", "Flags", "Current title", "Suggested title", "Job"].map((h) => (
                  <th
                    key={h}
                    style={{
                      textAlign: "left",
              ...
              {items.length === 0 ? (
                ...
                items.map((it) => (
                  <tr key={it.id}>
                    ...
                    <td style={tdStyle}>
                      {pill(String(it.titleQualityScore), it.titleQualityScore >= 85 ? "green" : it.titleQualityScore >= 65 ? "amber" : "red")}
                    </td>
                    <td style={tdStyle}>
                      {it.titleQualityFlags.length ? (
                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                          {it.titleQualityFlags.slice(0, 6).map((f) => pill(f, "slate"))}
                        </div>
                      ) : (
                    ...
                    <td style={tdStyle}>
                      <div style={{ fontWeight: 900 }}>{it.suggestedTitle}</div>
                      <div style={{ marginTop: 6, color: "rgba(226,232,240,0.55)", fontSize: 12 }}>
                        {it.wouldChange ? pill("would change", "amber") : pill("no change", "green")}
                        {it.suggestedFrom ? <span style={{ marginLeft: 8 }}>from {it.suggestedFrom}</span> : null}
                      </div>
                    </td>
                    <td style={tdStyle}>
                      <a href={`/jobs/${encodeURIComponent(it.id)}`} style={{ color: "rgba(56,189,248,0.95)", textDecoration: "none", fontWeight: 900 }}>
                        Open
                      </a>
                      <div style={{ marginTop: 6, color: "rgba(226,232,240,0.55)", fontSize: 12 }}>
                        <code>{it.id}</code>
