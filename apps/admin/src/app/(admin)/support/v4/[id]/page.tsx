...
export default function AdminSupportV4TicketPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleGptAutoReply = async () => {
    setGptLoading(true);
    setGptError(null);
    setSendError(null);
    try {
      const resp = await fetch(`/api/admin/v4/support/ticket/${encodeURIComponent(id)}/ai-reply`, {
        method: "POST",
      });
      const json = await resp.json().catch(() => null);
      if (!resp.ok || !json?.ok) {
        setGptError(String(json?.error?.message ?? json?.error ?? "Failed to generate reply"));
        return;
      }
      const suggested = json.data?.suggestedReply ?? "";
      if (suggested) setReply(suggested);
    } catch {
      setGptError("Failed to generate reply");
    } finally {
      setGptLoading(false);
    }
  };
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {ticket.status !== "CLOSED" ? (
          <div style={{ marginTop: 20, borderRadius: 12, border: "1px solid rgba(148,163,184,0.2)", background: "rgba(15,23,42,0.5)", padding: 16 }}>
            ...
            />
            {reply && (
              <p style={{ margin: "6px 0 0", fontSize: 11, color: "rgba(148,163,184,0.8)" }}>
                Suggested reply generated by GPT Auto-Reply — review before sending.
              </p>
            )}
            <div style={{ marginTop: 10, display: "flex", gap: 10, justifyContent: "flex-end" }}>
              <button
                onClick={() => void handleGptAutoReply()}
                disabled={gptLoading}
                style={{
                  borderRadius: 8,
                  border: "1px solid rgba(148,163,184,0.4)",
                  background: gptLoading ? "rgba(148,163,184,0.1)" : "transparent",
                  color: "rgba(226,232,240,0.9)",
                  padding: "8px 18px",
                  fontSize: 13,
                  fontWeight: 800,
              ...
              >
                {gptLoading ? "Generating reply…" : "GPT Auto-Reply"}
              </button>
              <button
