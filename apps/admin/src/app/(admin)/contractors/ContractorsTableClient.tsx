...
export function ContractorsTableClient({ rows, totalCount, page, pageSize, q, status, error, formatBadgeLabel }: Props) {
    ...
    <>
        ...
        <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0 }}>
          <thead>
            <tr>
              ...
              {["Name", "Email", "Phone", "Service Region", "Status", "Created", "Badges"].map((h) => (
                <th
                  key={h}
                  style={{
                    textAlign: "left",
            ...
            {error ? (
              ...
              rows.map((r) => (
                <tr key={r.id}>
                  ...
                  <td style={tdStyle}>
                    <a href={`/contractors/${encodeURIComponent(r.id)}`} style={linkStyle}>
                      {r.name ?? r.businessName ?? "\u2014"}
                    </a>
                    {r.businessName && r.businessName !== r.name ? (
                      <div style={{ marginTop: 4, fontSize: 12, color: "rgba(226,232,240,0.65)" }}>{r.businessName}</div>
                    ) : null}
                  </td>
      ...
      <div style={{ marginTop: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <a
            href={`/contractors${qs({ q: q || undefined, status: status || undefined, page: String(Math.max(1, page - 1)), pageSize: String(pageSize) })}`}
          ...
          <a
            href={`/contractors${qs({ q: q || undefined, status: status || undefined, page: String(Math.min(totalPages, page + 1)), pageSize: String(pageSize) })}`}
